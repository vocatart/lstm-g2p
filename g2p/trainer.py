...
from g2p.utils import BOS_IDX, UNK_IDX, PAD_IDX
from g2p.dataset import TSVDataset
from g2p.model import LstmG2P
...
class Trainer():
    ...
    def _train_epoch(self, step: int, total_loss: float, epoch: int, writer: SummaryWriter):
        ...
        running_loss = 0.0
        batch_count = 0
        pbar = tqdm.tqdm(self.training_loader, desc=f"epoch {epoch + 1}", unit="step")
